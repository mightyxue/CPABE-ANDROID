...
BASE_DIR=$(pwd)
NDK_PATH=~/android-ndk-r9d
~/android-ndk-r9d/build/tools/make-standalone-toolchain.sh --platform=android-9 --install-dir=$TOOLCHAIN_DIR
...
LDFLAGS=-L$BASE_DIR/gmp-6.0.0/build/lib ./configure --prefix=$(pwd)/build --host=arm-linux-androideabi --disable-shared
make
make install
cd ..
